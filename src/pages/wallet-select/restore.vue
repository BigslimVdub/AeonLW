...
<q-page>
    <div class="q-mx-md">
        ...
        <q-field>
            <q-btn color="primary" @click="restore_deterministic_wallet" label="Restore wallet" />
        </q-field>
        ...
        restore_deterministic_wallet() {
            ...
            if (this.$v.wallet.seed.$error) {
                this.$q.notify({
                    type: "negative",
                    timeout: 1000,
                    message: "Enter seed words to continue!"
                })
                return
            }

            let seed = this.wallet.seed.trim().replace(/\s{2,}/g, " ").split(" ")
            if(seed.length !== 24 && seed.length !== 25) {
                this.$q.notify({
                    type: "negative",
                    timeout: 1000,
                    message: "Invalid seed word length. Enter 24 or 25 word seed!"
                })
                return
            }
            ...
            this.$gateway.send("wallet", "restore_deterministic_wallet", this.wallet);
        },
        cancel() {
