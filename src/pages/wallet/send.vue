...
<q-page>
    <template v-if="view_only">
        ...
        <div class="row q-pt-sm q-mx-md q-mb-none items-center non-selectable" style="height: 44px;">
            ...
            <div class="col-8">
                <q-icon name="call_made" size="24px" /> Send Aeon
            </div>
    ...
    <template v-else>
        ...
        <div class="q-pa-md">
            ...
            <div class="row items-end gutter-md">
                ...
                <div>
                    <q-btn @click="newTx.amount = unlocked_balance / 1e12" :text-color="theme=='dark'?'white':'dark'">All coins</q-btn>
                </div>
...
export default {
    computed: mapState({
        ...
        is_ready (state) {return this.$store.getters["gateway/isReady"]},
        is_able_to_send (state) {return this.$store.getters["gateway/isAbleToSend"]}
    }),
    data () {
        return {
            sending: false,
    ...
    methods: {
        ...
        send: function () {
            ...
            if(this.newTx.amount < 0) {
                ...
                return
            } else if(this.newTx.amount > 5000) {
                this.$q.notify({
                    type: "negative",
                    timeout: 2000,
