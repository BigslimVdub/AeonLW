...
if (process.env.PROD) {
    ...
    global.__aeon_bin = require("path").join(__dirname, "..", "bin").replace(/\\/g, "\\\\")
} else {
    global.__aeon_bin = require("path").join(process.cwd(), "bin").replace(/\\/g, "\\\\")
}
            ...
            portscanner.checkPortStatus(config.port, "127.0.0.1", (error, status) => {
                if (status === "closed") {
                    ...
                    dialog.showMessageBox(mainWindow, {
                        title: "Startup error",
                        message: `An Aeon wallet is already open, or port ${config.port} is in use`,
                        type: "error",
                        buttons: ["ok"]
                    }, () => {
                        showConfirmClose = false
