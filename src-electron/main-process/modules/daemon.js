...
export class Daemon {
    ...
    checkVersion() {
        return new Promise((resolve, reject) => {
            if (process.platform === "win32") {
                let aeond_path = path.join(__aeon_bin, "aeond.exe")
                let aeond_version_cmd = `"${aeond_path}" --version`
                if (!fs.existsSync(aeond_path))
                    resolve(false)
                child_process.exec(aeond_version_cmd, (error, stdout, stderr) => {
                    if(error)
                        resolve(false)
                    resolve(stdout)
                })
            } else {
                let aeond_path = path.join(__aeon_bin, "aeond")
                let aeond_version_cmd = `"${aeond_path}" --version`
                if (!fs.existsSync(aeond_path))
                    resolve(false)
                child_process.exec(aeond_version_cmd, {detached: true}, (error, stdout, stderr) => {
                    if(error)
                        resolve(false)
                    resolve(stdout)
                })
    ...
    start(options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(options.app.testnet) {
                this.testnet = true
                args.push("--testnet")
                args.push("--log-file", path.join(options.app.data_dir, "testnet", "logs", "aeond.log"))
           // } else {
                //args.push("--log-file", path.join(options.app.data_dir, "logs", "aeond.log"))
            }
            ...
            if(options.daemon.type === "local_remote" && !options.app.testnet) {
                args.push("--testnet", "--bootstrap-daemon-address", `${options.daemon.remote_host}:${options.daemon.remote_port}`)
                args.push("--log-file", path.join(options.app.data_dir, "testnet", "logs", "aeond.log")) //logging may be turned off for remote daemon,
                //this may cause an error or rpc not to connect properly
            }

            if (process.platform === "win32") {
                this.daemonProcess = child_process.spawn(path.join(__aeon_bin, "aeond.exe"), args)
            } else {
                this.daemonProcess = child_process.spawn(path.join(__aeon_bin, "aeond"), args, {
                    detached: true
                })
            }
    ...
    startHeartbeat() {
        ...
        }, this.local ? 5 * 1000 : 1 * 1000) // 5 seconds for local daemon, 1 seconds for remote
        this.heartbeatAction()
